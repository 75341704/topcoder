...
// CUT begin
	public static void main(String[] args){
		System.err.println("CheeseRolling (1000 Points)");
		System.err.println();
	...
	static void runTest(HashSet<Integer> caseSet) {
	    ...
	    while (true) {
            ...
            String[] wins = new String[Integer.parseInt(Reader.nextLine())];
            for (int i = 0; i < wins.length; ++i)
                wins[i] = Reader.nextLine();
            Reader.nextLine();
            ...
            if (doTest(wins, __answer))
                passed++;
	    }
	    if (caseSet.size() > 0) cases = caseSet.size();
        System.err.println(String.format("%nPassed : %d/%d cases", passed, cases));

        int T = (int)(System.currentTimeMillis() / 1000) - 1437704924;
	...
	static boolean doTest(String[] wins, long[] __expected) {
		for (int i = 0; i < wins.length; i++) {
			wins[i] = new String(wins[i]);
		}
		...
		try {
			__result = instance.waysToWin(wins);
		}
	...
	static class Reader {
        private static final String dataFileName = "CheeseRolling.sample";
